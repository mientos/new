document.addEventListener('DOMContentLoaded', () => {
    ...
    // ZMIANA: Przechwycenie elementów do kontroli muzyki tła
    ...
    // ZMIANA: Ustawienie niskiej głośności dla muzyki w tle
    backgroundMusicPlayer.volume = 0.08;
    ...
    const stopCurrentAudio = (resetButton = true) => {
        ...
        // ZMIANA: Muzyka w tle nie jest już tutaj zatrzymywana
    ...
    const handleReadAloudClick = () => {
        ...
        voicePlayer.play().then(() => {
            readAloudBtn.disabled = false;
            // ZMIANA: Już nie włączamy tutaj muzyki tła
        }).catch(error => {
    ...
    // ZMIANA: Nowa logika do obsługi przycisku muzyki w tle
    toggleMusicBtn.addEventListener('click', () => {
        if (backgroundMusicPlayer.paused) {
            backgroundMusicPlayer.play().catch(e => {
                // Ta obsługa jest ważna, bo przeglądarki mogą blokować autoplay
